...
function handleChange(e){
  const t=e.target;
  if(t.dataset.tokenToggle){ runtime.player.drafts.useToken = !!t.checked; persistPlayer(); }
  if(t.dataset.mute) setMuted(!!t.checked);
}
...
function renderTokenChooser(s){const available=s.self.tokens>0;return `<div class="token-box"><div class="token-toggle"><div><b>🧶 Masz ${s.self.tokens} ${pluralToken(s.self.tokens)}.</b><div class="small">Żeton podwaja punkty zdobyte za ten głos. Jeśli go nie użyjesz, każdy pozostały da +1 pkt w finale.</div></div><label class="switch"><input type="checkbox" data-token-toggle ${runtime.player.drafts.useToken?"checked":""} ${available?"":"disabled"}><span></span></label></div></div>`;}
